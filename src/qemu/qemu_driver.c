                                    ...
                                    unsigned int flags)
{
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG) {
        ...
        for (i = 0; i < devlist->count; i++)
            if (virDomainDefCompatibleDevice(vmdef, devlist->devs[i]) < 0)
            goto cleanup;
        for (i = 0; i < devlist->count; i++)
            if ((ret = qemuDomainAttachDeviceConfig(vmdef, devlist->devs[i], conn, caps,
                                                    parse_flags,
                                                    driver->xmlopt)) < 0)
                goto cleanup;
    }
    ...
    if (flags & VIR_DOMAIN_AFFECT_LIVE) {
        for (i = 0; i < devcopylist->count; i++)
            if (virDomainDefCompatibleDevice(vm->def, devcopylist->devs[i]) < 0)
        ...
        if (VIR_ALLOC(rollbacklist) < 0)
            goto undoprepare;
        for (i = 0; i < devcopylist->count; i++) {
            if (virDomainDeviceDefListAddCopy(rollbacklist, devcopylist->devs[i],
                                              vm->def, caps, driver->xmlopt) < 0)
                goto rollback_livehotplug;
            if ((ret = qemuDomainAttachDeviceLive(vm, devcopylist->devs[i], conn, driver)) < 0)
                goto rollback_livehotplug;
        }
        /*
 ...
 rollback_livehotplug:
    ...
    for (j = 0; j < rollbacklist->count-1; j++) {
        if ((ret = qemuDomainDetachDeviceLive(vm, rollbacklist->devs[j], driver)) < 0) {
 ...
 undoprepare:
    if (pciHostdevs) {
        ...
        for (j = rollbacklist ? rollbacklist->count-1 : 0; j < d; j++)
                                       ...
                                       unsigned int flags)
{
    ...
    if (multifunction) {
        ...
        if (!dev || VIR_ALLOC(devlist) < 0)
            goto endjob;
        if (virDomainDeviceDefListAddCopy(devlist, dev, vm->def, caps,
                                                  driver->xmlopt) < 0)
            goto endjob;
    }
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG &&
        ...
        if (VIR_ALLOC(devcopylist) < 0)
            goto endjob;
        for (i = 0; i < devlist->count; i++)
            if (virDomainDeviceDefListAddCopy(devcopylist, devlist->devs[i], vm->def,
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG) {
        ...
        for (i = 0; i < devlist->count; i++)
            if ((ret = qemuDomainUpdateDeviceConfig(vmdef, devlist->devs[i], caps,
                                                    parse_flags,
                                                    driver->xmlopt)) < 0)
                goto endjob;
    }
                                    ...
                                    unsigned int flags)
{
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG &&
        ...
        if (VIR_ALLOC(devcopylist) < 0)
            goto cleanup;
        for (i = 0; i < devlist->count; i++)
            if (virDomainDeviceDefListAddCopy(devcopylist, devlist->devs[i], vm->def, caps,
                                              driver->xmlopt) < 0)
                goto cleanup;
    }
    ...
    if (flags & VIR_DOMAIN_AFFECT_CONFIG) {
        ...
        for (i = 0; i < devlist->count; i++)
            if ((ret = qemuDomainDetachDeviceConfig(vmdef, devlist->devs[i], caps,
                                                    parse_flags,
                                                    driver->xmlopt)) < 0)
            goto cleanup;
    }
    ...
    if (flags & VIR_DOMAIN_AFFECT_LIVE) {
        for (i = 0; i < devcopylist->count; i++) {
            if ((ret = qemuDomainDetachDeviceLive(vm, devcopylist->devs[i], driver)) < 0)
                goto cleanup;
